...
- [x] Add FFI bindings for `ext2fs_new_inode`, `ext2fs_link`, `ext2fs_unlink`, `ext2fs_mkdir`, `ext2fs_file_write`, `ext2fs_write_inode`, and `ext2fs_flush`.
- [x] Implement safe Rust methods on `Ext2FsHandle` for creating files, writing bytes, deleting files, and creating directories.
- [x] Implement compulsory `flush()` method ensuring block allocation and inode bitmaps are committed to disk.
- [x] Unit tests verifying file creation and write persistence on test disk images.
