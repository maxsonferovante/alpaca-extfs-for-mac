...
- [ ] Implement `fuser` callbacks for `create`, `write`, `unlink`, `mkdir`, `rmdir`, `flush`, and `release`.
- [ ] Wrap `Ext2FsHandle` in `Arc<Mutex<...>>` to synchronize concurrent Finder requests safely.
- [ ] Trigger `ext2fs_flush` after write, unlink, or mkdir operations.
- [ ] Implement signal handler catching `SIGINT`/`SIGTERM` to call `ext2fs_flush` and `fuse_unmount` cleanly.
- [ ] Verify creating, modifying, and deleting files directly inside macOS Finder.
