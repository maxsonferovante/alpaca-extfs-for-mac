...
- [ ] FFI module defining C bindings to `ext2fs_open`, `ext2fs_close`, `ext2fs_read_inode`, `ext2fs_dir_iterate2`, and `ext2fs_file_open`/`read`.
- [ ] `Ext2FsHandle` struct providing safe Rust wrappers around raw `ext2_filsys` pointers.
- [ ] Methods to list files in a directory path and query inode attributes (size, permissions, timestamps).
- [ ] Unit test opening a sample Ext4 disk image and listing its root directory.
