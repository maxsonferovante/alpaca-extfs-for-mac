...
- [x] Implement `fuser::Filesystem` trait callbacks for `lookup`, `getattr`, `readdir`, `open`, and `read`.
- [x] Implement CLI command parsing (`mount_ext4 <device> <mount_point>`) validating root/sudo execution and raw disk device path (`/dev/rdisk`).
- [x] Mount Ext4 image/device to macOS mount point using macFUSE.
- [x] Finder displays directory contents and reads file content cleanly.
